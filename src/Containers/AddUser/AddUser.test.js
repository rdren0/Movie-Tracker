...
describe('AddUser', () => {
  ...
  it('should have default states', () =>{
    expect(wrapper.state()).toEqual({ email: '', password: '', name: '', status: '' })
  })

  it('should update state when typing', () => {
    expect(wrapper.state().email).toEqual('');
    let mockEvent = {target:{value:'123@email.com',
                      name: 'email'}}
  ...
  it('fetch post new user data to server', () => {
    ...
    expect(fetch).toBeCalledWith(url,{"body": "{\"email\":\"\",\"password\":\"\",\"name\":\"\",\"status\":\"\"}", "headers": {"Content-Type": "application/json"}, "method": "POST"})
  })
